...
#define MAKE_HOOK(name, address, type, callconvention, ...) namespace H \
{\
	namespace name\
	{\
		void Initialize();\
...
void H::name::Initialize() { assert(address != nullptr); Hook.CreateHook(reinterpret_cast<void*>(address), reinterpret_cast<void*>(Detour)); }\
